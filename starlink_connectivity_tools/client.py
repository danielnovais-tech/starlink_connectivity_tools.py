...
class StarlinkClientV1:
    """
    Legacy client for interacting with Starlink user terminals.

    This class provides methods to retrieve status, configure settings,
    and manage Starlink dishes either locally or remotely.

    Args:
        target: Connection target (default: "192.168.100.1:9200" for local
            connection)
    ...
    def connect(self) -> None:
        ...
        if self._channel is None:
            ...
            if use_secure:
                ...
                # generated gRPC stub from the Starlink proto files
...
class StarlinkClientV2:
    """
    Starlink client for device management and monitoring.

    Provides methods for device management, network monitoring, and
    configuration. Supports both local (direct) and remote (via
    Starlink API) connections.

    Args:
        host: Hostname or IP address of the Starlink device
            (default: 192.168.100.1)
        port: gRPC port (default: 9200)
        use_remote: Whether to use remote API instead of local connection
        api_key: API key for remote authentication
            (required if use_remote=True)
...
class StarlinkDishClient:
    ...
    def __init__(
        ...
        Args:
            ...
            insecure: Whether to use insecure channel. Defaults to True for
                local access.
        ...
        self.timeout = timeout
        self.use_remote = bool(session_cookie)
        self.auth_token = session_cookie
        self._channel: Optional[grpc.Channel] = None
    ...
    def connect(self) -> None:
        ...
        # Note: Authentication should be handled via metadata, not
        # channel options
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to connect to Starlink dish at {self.address} "
    ...
    def get_dish_status(self) -> Dict[str, Any]:
        ...
        Returns information about connectivity, alerts, uptime, and current
        state.
    ...
    def get_history_samples(self, samples: int = 300) -> Dict[str, Any]:
        """
    ...
    def get_network_stats_dict(self) -> Dict[str, Any]:
        """
        Get current network statistics as a dictionary.

        Returns network performance metrics including speeds, latency,
        and packet loss.
    ...
    def get_dish_telemetry(self, streaming: bool = False) -> Dict[str, Any]:
        """
        Retrieve device telemetry including alerts, errors, and warnings.

        Args:
            streaming: If True, would return a streaming iterator.
                Currently not implemented in this placeholder version
                (default: False)
    ...
    def change_wifi_config(self,
        ...
        if password is not None:
            # Indicate password was updated without exposing it
            config["password_updated"] = "true"
        if bypass_mode is not None:
            config["bypass_mode"] = str(bypass_mode).lower()
    ...
    def get_status(self) -> DeviceStatus:
        ...
        Returns:
            DeviceStatus: Current device status including connectivity and
                alerts
    ...
    def get_history(
        ...
        Example:
            ...
            ...         latency = entry.network_stats.latency_ms
            ...         print(f"{entry.timestamp}: {latency}ms")
        """
        if not self.use_remote and not self._channel:
            raise StarlinkConnectionError(
                "Not connected. Call connect() first."
    ...
    def get_telemetry(self) -> TelemetryData:
        ...
        Example:
            ...
            ...     alerts = telemetry.get_alerts_by_level(
            ...         AlertLevel.CRITICAL
            ...     )
            ...     for alert in alerts:
    ...
    async def stream_telemetry(self) -> AsyncIterator[TelemetryData]:
        ...
        try:
            ...
            raise StarlinkOperationError(
                f"Failed to stream telemetry: {str(e)}"
            )
    ...
    def set_dish_config(self, config: DishConfig) -> bool:
        ...
        try:
            ...
            raise StarlinkOperationError(
                f"Failed to set dish config: {str(e)}"
            )
    ...
    def get_dish_config(self) -> DishConfig:
        ...
        try:
            ...
            raise StarlinkOperationError(
                f"Failed to get dish config: {str(e)}"
            )
    ...
    def get_device_location(self) -> DeviceLocation:
        ...
        Returns precise location when connected locally, or H3 cell when
        remote.
        ...
        try:
            ...
            raise StarlinkOperationError(
                f"Failed to get device location: {str(e)}"
            )
    ...
    def get_wifi_status(self) -> WiFiStatus:
        ...
        try:
            ...
            raise StarlinkOperationError(
                f"Failed to get WiFi status: {str(e)}"
            )
    ...
    def set_wifi_config(self, config: WiFiConfig) -> bool:
        ...
        Example:
            >>> config = WiFiConfig(
            ...     ssid="MyStarlink", password="NewPassword123"
            ... )
        ...
        try:
            ...
            raise StarlinkOperationError(
                f"Failed to set WiFi config: {str(e)}"
            )
    ...
    def get_account_data(self) -> AccountData:
        ...
        Raises:
            StarlinkOperationError: If the operation fails or not using
                remote connection
        ...
        if not self.use_remote:
            raise StarlinkOperationError(
                "Account data is only available via remote API"
            )
        ...
        try:
            ...
            raise StarlinkOperationError(
                f"Failed to get account data: {str(e)}"
            )
    ...
    def discover_services(self) -> List[str]:
        ...
        try:
            reflection_stub = (
                reflection_pb2_grpc.ServerReflectionStub(self._channel)
            )
...
class StarlinkClient:
    ...
    def __init__(
        self,
        base_url: str = "https://api.starlink.com",
        api_key: Optional[str] = None
    ):
        """
