...
class ConnectivityDiagnostics:
    ...
    def run_full_diagnostic(self) -> Dict[str, Any]:
        ...
        failed_tests = [
            name
            for name, result in diagnostic["tests"].items()
    ...
    def _initialize_diagnostic_result(self) -> Dict:
        """Initialize diagnostic result structure"""
        diagnostic: Dict[str, Any] = {
            "timestamp": datetime.now().isoformat(),
            "tests": {},
            "summary": {"status": "unknown", "issues_found": 0, "recommendations": []},
        }
    ...
    def _check_network_interfaces(self) -> Dict:
        ...
        interfaces: Dict[str, Any] = {"interfaces": [], "issues": [], "healthy": False}
        ...
        try:
                    ...
                    if netifaces.AF_INET in addrs:
                        for addr in addrs[netifaces.AF_INET]:
                            iface_info["addresses"].append(
                                {"ip": addr.get("addr"), "netmask": addr.get("netmask")}
                            )
            ...
            has_ip = any(
                len(iface["addresses"]) > 0 for iface in interfaces["interfaces"]
            )
    ...
    def _check_dns_resolution(self) -> Dict:
        ...
        results: Dict[str, Any] = {"domains": {}, "healthy": False, "issues": []}
    ...
    def _check_gateway_connectivity(self) -> Dict:
        ...
        try:
            ...
            has_default = "default" in gateways
            has_inet = has_default and netifaces.AF_INET in gateways["default"]
            if has_inet:
    ...
    def _check_internet_connectivity(self) -> Dict:
        ...
        results: Dict[str, Any] = {"hosts": {}, "healthy": False, "issues": []}
    ...
    def _check_satellite_status(self) -> Dict:
        ...
        try:
            ...
            status["signal_quality"] = random.choice(
            ...
            status["healthy"] = (
                status["connected"]
                and status["signal_quality"] in ["excellent", "good"]
                and status["obstruction_percent"] < 5
            )
            ...
            if not status["healthy"]:
                ...
                if status["signal_quality"] in ["fair", "poor"]:
                    status["issues"].append(
                        f'Poor signal quality: {status["signal_quality"]}'
                    )
                if status["obstruction_percent"] >= 5:
                    obstruction_msg = (
                        f"High obstruction: " f'{status["obstruction_percent"]:.1f}%'
                    )
                    status["issues"].append(obstruction_msg)
        ...
        if issues:
            if any("DNS" in issue for issue in issues):
                recommendations.append(
                    "Try using alternative DNS servers (1.1.1.1, 8.8.8.8)"
                )
            ...
            if any("Satellite" in issue for issue in issues):
                recommendations.append(
                    "Reposition satellite dish for better line of sight"
                )
    ...
    def get_historical_diagnostics(self, hours: int = 24) -> List[Dict]:
    ...
    def generate_health_report(self) -> Dict:
        ...
        healthy_count = sum(1 for d in recent if d["summary"]["status"] == "healthy")
        health_percentage = (
            (healthy_count / total_diagnostics * 100) if total_diagnostics > 0 else 0
        )
...
class StarlinkDiagnostics:
    ...
    def get_system_health(self):
        ...
        return {"status": "unknown", "alerts": self.alerts}
