...
def create_alerts_table(diagnostic_data: dict) -> Table:
    ...
    if not diagnostic_data.get("alerts"):
        table.add_row("None", "[green]info[/green]", "All systems normal", "Continue monitoring")
...
@cli.command()
@click.option('--target', default=None, help='Starlink gRPC target (default: 192.168.100.1:9200)')
...
def status(target, simulation):
    ...
    try:
        ...
        console.print(f"Downlink: {status_data.get('downlink_throughput_bps', 0) / 1_000_000:.1f} Mbps")
        console.print(f"Uplink: {status_data.get('uplink_throughput_bps', 0) / 1_000_000:.1f} Mbps")
        ...
        console.print(f"Obstructed: [{obs_color}]{obstructed}[/{obs_color}] ({obs_pct:.1f}%)")
...
@click.option('--scenario', type=click.Choice(['normal', 'humanitarian', 'medical', 'disaster', 'conflict']), 
              default='normal', help='Monitoring scenario')
@click.option('--simulation', is_flag=True, help='Use simulation mode')
def monitor(interval, scenario, simulation):
    ...
    try:
        ...
        console.print(f"[green]Monitoring started[/green] (scenario: {scenario}, interval: {interval}s)")
        ...
        try:
            while True:
                ...
                console.clear()
                console.print(f"[bold]Starlink Monitor[/bold] - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                console.print(f"Scenario: [cyan]{scenario}[/cyan] | Status: [{get_status_color(health_data['status'])}]{health_data['status']}[/{get_status_color(health_data['status'])}]\n")
